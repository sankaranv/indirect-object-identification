...
def compute_means(
    ...
    N, seq   = corrupted_toks.shape

    means = torch.zeros(n_layers, N, seq, n_heads, d_head)
...
def run_with_mean_ablation(
    ...
    n_heads  = model.config.n_head
    d_head   = model.config.n_embd // n_heads
    N, seq   = clean_toks.shape

    keep: Dict[int, torch.Tensor] = {
        l: torch.zeros(N, seq, n_heads, dtype=torch.bool) for l in range(n_layers)
    }
    for head_type, head_list in circuit.items():
        positions = word_idx[seq_pos_to_keep[head_type]]
        for l, h in head_list:
            keep[l][torch.arange(N), positions, h] = True
    ...
    with model.trace({"input_ids": clean_toks}):
        for layer in range(n_layers):
            ...
            model.transformer.h[layer].attn.c_proj.input[:] = z_new.reshape(N, seq, n_heads * d_head)
