...
HEAD_COLOR: dict = {}
for _lh in NM_HEADS:  HEAD_COLOR[_lh] = "#2196F3"  # blue
for _lh in NNM_HEADS: HEAD_COLOR[_lh] = "#F44336"  # red
for _lh in BNM_HEADS: HEAD_COLOR[_lh] = "#64B5F6"  # light blue
for _lh in SI_HEADS:  HEAD_COLOR[_lh] = "#FF9800"  # orange
_OTHER_COLOR = "#BDBDBD"
            ...
            with model.trace({"input_ids": clean_toks}):
                ...
                for l in range(n_layers):
                    if l == sl:
                        ...
                        if nm_by_layer.get(l):
                            for nm_h in nm_by_layer[l]:
                                if nm_h != sh:
                                    nm_sl = slice(nm_h * d_head, (nm_h + 1) * d_head)
                                    model.transformer.h[l].attn.c_proj.input[..., nm_sl] = (
                                        nm_means[(l, nm_h)]
                                    )
                    else:
                        if nm_by_layer.get(l):
                            ...
                            z_comb = clean_z[l].clone()
                            for nm_h in nm_by_layer[l]:
                                nm_sl = slice(nm_h * d_head, (nm_h + 1) * d_head)
                                z_comb[..., nm_sl] = nm_means[(l, nm_h)]
                            model.transformer.h[l].attn.c_proj.input[...] = z_comb
                        else:
...
def _bar_chart(scores: dict, title: str, fname: str) -> None:
    ...
    labels  = [f"{l}.{h}" for (l, h), _ in items]
    effects = [v for _, v in items]
    colors  = [HEAD_COLOR.get((l, h), _OTHER_COLOR) for (l, h), _ in items]
...
def run() -> None:
    ...
    # ── Before NM knockout ───────────────────────────────────────────────────
    print("Running patching loop (before NM knockout)…")
    ...
    print("Computing NM head means from ABC dataset…")
    ...
    # ── After NM knockout ────────────────────────────────────────────────────
    print("Running patching loop (after NM knockout)…")
