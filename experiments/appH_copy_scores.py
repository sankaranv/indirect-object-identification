...
def run() -> None:
    ...
    labels = [f"L{l}H{h}" for l, h in top20]
    values = [scores[k] for k in top20]
    colors = [CIRCUIT_COLORS[_head_circuit_type(l, h)] for l, h in top20]

    os.makedirs("plots/copy_scores", exist_ok=True)

    fig, ax = plt.subplots(figsize=(12, 5))
    bars = ax.bar(range(len(top20)), values, color=colors)
    ...
    for l, h in sorted_heads[:10]:
        ct = _head_circuit_type(l, h)
        print(f"  L{l}H{h:2d}  {scores[(l,h)]:+.4f}  [{ct}]")
