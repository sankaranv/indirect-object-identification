...
def _ablate_circuit(base_circuit, heads_to_remove):
    ...
    return {
        t: [lh for lh in heads if lh not in heads_to_remove]
        for t, heads in base_circuit.items()
    }


def _logit_diff(logits, N, end_pos, io_ids, s_ids):
    return (
        logit_diff(
...
def _model_logit_diff_ablate_heads(model, ioi, means, K):
    ...
    for layer, head in K:
        keep[layer][:, :, head] = False
    layers_to_patch = [layer for layer in range(n_layers) if not keep[layer].all()]
...
def run():
    ...
    full_logit_diff = _logit_diff(
        full_logits, N, end_pos, ioi.io_tokenIDs, ioi.s_tokenIDs
    )
    print(f"Full model LD: {full_logit_diff:.4f}")
    ...
    for scatter, fname, title in [
        ...
        (
            scatter_naive,
            "fig6b_scatter.png",
            "Completeness scatter: naive circuit (NM+SI only)",
        ),
    ]:
        if not scatter:
            continue
