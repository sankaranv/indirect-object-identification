...
def run(threshold: float = 0.2, ioi_threshold: float = 0.1, seq_len: int = 50, batch: int = 50):
    model  = load_model()
    ...
    dt_h = dt_h | {(l, h) for (l, h), v in dt_ioi.items() if v > ioi_threshold}
    ih_h = ih_h | {(l, h) for (l, h), v in ih_ioi.items() if v > ioi_threshold}
