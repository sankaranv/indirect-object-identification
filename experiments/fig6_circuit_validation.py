...
def run():
    ...
    print("Computing ABC means by template…")
    ...
    print(f"\n[Figure 6]")
    print(f"  Full:              {full_ld:.4f}  (paper ~3.55)")
    print(f"  Circuit only:      {faithful_ld:.4f}  (paper ~3.26)")
    print(f"  Circuit ablated:   {complete_ld:.4f}  (paper ~-0.38)")
    print(f"  Faithfulness:      {faith_ratio:.1%}  (target >70%)")
    print(f"  Completeness rem.: {compl_ratio:.1%}  (target <30%)")
    ...
    assert faith_ratio > 0.70, f"Faithfulness {faith_ratio:.1%} < 70%"
    assert compl_ratio < 0.30, f"Completeness remainder {compl_ratio:.1%} > 30%"
    print("PASS")
